...
# Políticas de acceso específicas por componente
resource "vault_policy" "database_policy" {
  name = "database-policy"

  policy = <<EOT
# Política para el componente de base de datos
...
resource "vault_policy" "identity_policy" {
  ...
  policy = <<EOT
# Política para el componente de identidad
...
resource "vault_policy" "storage_policy" {
  name = "storage-policy"

  policy = <<EOT
# Política para el componente de almacenamiento
path "secret/data/storage/*" {
  capabilities = ["read"]
}

path "secret/metadata/storage/*" {
  capabilities = ["read", "list"]
}
EOT
}
...
resource "vault_policy" "monitoring_policy" {
  ...
  policy = <<EOT
# Política para el componente de monitoreo
...
# Roles de autenticación de Kubernetes
resource "vault_kubernetes_auth_backend_role" "database_role" {
  backend                          = "kubernetes"
  role_name                        = "database-role"
  bound_service_account_names      = ["postgres", "database-sa"]
  bound_service_account_namespaces = ["database"]
  token_ttl                        = 3600
  token_policies                   = ["database-policy"]
}

resource "vault_kubernetes_auth_backend_role" "identity_role" {
  backend                          = "kubernetes"
  role_name                        = "identity-role"
  bound_service_account_names      = ["zitadel", "identity-sa"]
  bound_service_account_namespaces = ["identity"]
  token_ttl                        = 3600
  token_policies                   = ["identity-policy"]
}

resource "vault_kubernetes_auth_backend_role" "storage_role" {
  backend                          = "kubernetes"
  role_name                        = "storage-role"
  bound_service_account_names      = ["minio", "storage-sa"]
  bound_service_account_namespaces = ["storage"]
  token_ttl                        = 3600
  token_policies                   = ["storage-policy"]
}

resource "vault_kubernetes_auth_backend_role" "monitoring_role" {
  backend                          = "kubernetes"
  role_name                        = "monitoring-role"
  bound_service_account_names      = ["grafana", "monitoring-sa"]
